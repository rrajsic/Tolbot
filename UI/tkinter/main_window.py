...
class TkinterUI(UI):
    ...
    def start_main_window(self, user):
        ...
        self.welcome_label = CTkLabel(
            self.root, text=f"Hi, {self.user['username']}!", font=("Arial", 22)
        )
        self.welcome_label.place(relx=0.05, rely=0.05, anchor="w")

        self.team_name_label = CTkLabel(
            self.root, text="Team name:", font=("Arial", 16)
        )
        ...
        self.project_name_label = CTkLabel(
            self.root, text="Project name:", font=("Arial", 16)
        )
        ...
        export_button = CTkButton(
            self.root, text="Export", command=self.export, font=("Arial", 22)
        )
    ...
    def create_gerrit_widgets(self):
        ...
        self.server_entry[0] = CTkComboBox(
            self.root, values=server_values, width=300, font=("Arial", 14)
        )
        if server_values:
        ...
        self.change_id_label_1 = CTkLabel(
            self.root, text="Change ID:", font=("Arial", 16)
        )
        ...
        self.revision_id_label_1 = CTkLabel(
            self.root, text="Revision ID:", font=("Arial", 16)
        )
        ...
        self.server_entry[1] = CTkComboBox(
            self.root, values=server_values, width=300, font=("Arial", 14)
        )
        if server_values:
        ...
        self.change_id_label_2 = CTkLabel(
            self.root, text="Change ID:", font=("Arial", 16)
        )
        ...
        self.revision_id_label_2 = CTkLabel(
            self.root, text="Revision ID:", font=("Arial", 16)
        )
        ...
        self.server_entry[2] = CTkComboBox(
            self.root, values=server_values, width=300, font=("Arial", 14)
        )
        if server_values:
        ...
        self.change_id_label_3 = CTkLabel(
            self.root, text="Change ID:", font=("Arial", 16)
        )
        ...
        self.revision_id_label_3 = CTkLabel(
            self.root, text="Revision ID:", font=("Arial", 16)
        )
    ...
    def export(self, event=None):
        ...
        for i in range(3):
            if self.change_id_entry[i].get() != "":
                repositories[i] = self.repository_factory.create_repository(
                    self.server_entry[i].get(),
                    self.change_id_entry[i].get(),
                    self.revision_id_entry[i].get(),
                    self.user,
                )
    ...
    def run(self):
        while True:
            if self.repository_factory.get_servers() == None:
                print("Error: no servers configured")
                sys.exit()
            self.login = LoginUI(self.root, self.repository_factory.get_servers()[0])
    ...
    def notify_login_exception(self, err):
        if isinstance(err, requests.exceptions.HTTPError):
            ...
            messagebox.showerror(
                "LoginUI Failed", f"Error connecting to Gerrit server: {err}"
            )
            ...
            messagebox.showerror(
                "LoginUI Failed", f"An unexpected error occurred: {err}"
            )
